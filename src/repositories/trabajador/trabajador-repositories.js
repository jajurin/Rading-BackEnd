...
async function buscarCliente(texto, ids = []) {
...
async function filtrarCl(estrellas, necesidad, distancia, horario, precio, ubicacion, tipo) {
    const client = new Client(config)
    await client.connect()
...
async function mostrarTrabajosRealizados(idTrabajador) {
    const client = new Client(config)
    await client.connect()
...
export { buscarCliente, filtrarCl, mostrarTrabajosRealizados }
